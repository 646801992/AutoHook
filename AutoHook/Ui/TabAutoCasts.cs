...
internal class TabAutoCasts : TabBaseConfig
{
    ...
    public override void DrawHeader()
    {
        ...
        {
            ImGui.SameLine();
            if (DrawUtil.Checkbox("总是以小钓大", ref cfg.DontCancelMooch, "如果是以小钓大的机会并且自动以小钓大启用是，任何会取消以小钓大机会的技能不会被使用。 (例如：撒饵，鱼眼，大鱼猎手等)"))
            {
                Service.Configuration.Save();
            }
        }
    ...
    {
        if (ImGui.Checkbox("使用以小钓大II", ref cfg.EnableMooch2))
        {
            Service.Configuration.Save();
        }
